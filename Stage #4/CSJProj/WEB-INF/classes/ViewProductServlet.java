...
public class ViewProductServlet extends HttpServlet {
	...
	private String getReviews(HashMap<String, ArrayList<Review>> reviews, String productId) {
		...
		if (review.size() == 0) {
			...
			for (Review r : review) {
				if (isSet) {
					_Content += "" + "<div class='row'>" + "<div class='col-md-3'>"
							+ "<label class='dark'>Retailer:</label>" + r.getRetailername() + "  " + "</div>"
							+ "<div class='col-md-3'>" + "<label class='dark'>City:</label>" + r.getRetailerCity()
							+ "   " + "</div>" + "<div class='col-md-3'>" + "<label class='dark'>State:</label>"
							+ r.getRetailerState() + "  " + "</div>" + "<div class='col-md-3'>"
							+ "<label class='dark'>Zip:</label>" + r.getRetailerzip() + "</div>" + "</div>";
					isSet = false;
				}
				...
				for (int i = 0; i < rating; i++) {
					_Content += "<i class='fa fa-star' aria-hidden='true'></i>";
				}
				_Content += "</p>" + "<p>" + r.getReview() + "</p>" + "<footer class='user-info'>" + r.getUsername()
						+ " " + "<cite title='Source Title'>" + "(" + r.getGender() + ") " + "Age: " + r.getAge() + " "
						+ r.getOccupation() + " by occupation" + "--" + new Date(r.getTime()).toString() + "</cite>"
						+ "</footer>" + "</blockquote>";
			}
		}
	...
	private void storeDataToMongoDB(HttpServletRequest request, HttpServletResponse response, PrintWriter out,
		...
		Review rev = new Review(prodId, productName, productPrice, manufacturer, productOnSale, retailername,
				retailerzip, retailercity, retailerstate, username, age, gender, occupation, rating, review, time);
		mdsu.storeReview(rev);
	}

	private String getReviewForm(String productId) {
		String _Content = "<form class='review-form' method='get' action='/CSJProj/ViewProductServlet'>"
				+ "<input type='hidden' name='productId' value='" + productId + "'>"
				+ "<input type='hidden' name='reviewform' value='true'>" + "<center>"
				+ "<h2 class='form-signin-heading'>Write Review about Products</h2>" + "</center>"
				+ "<div class='form-group'>" + "<div class='row'>" + "<div class='col-md-12'>"
				+ "<label class='dark'>Retailer Name:</label>"
				+ "<input type='text'  value='Smartportables' name='retailername'>" + "</div>" + "</div>"
				+ "<div class='row'>" + "<div class='col-md-4'>" + "<label class='dark'>Zip:</label>"
				+ "<input type='text'	name='retailerzip' value='60120'>" + "</div>" + "<div class='col-md-4'>"
				+ "<label class='dark'>City:</label>" + "<input type='text' name='retailercity' value='Chicago'>"
				+ "</div>" + "<div class='col-md-4'>" + "<label class='dark'>State:</label>"
				+ "<input type='text' name='retailerstate' value='IL'>" + "</div>" + "</div>" + "</div>"
				+ "<div class='form-group'>" + "<div class='row'>" + "<div class='col-md-12'>"
				+ "<label class='dark'>Reviewer Name:</label> " + "<input type='text' name='reviewername'>" + "</div>"
				+ "</div>" + "<div class='row'>" + "<div class='col-md-4'>" + "<label class='dark'>Your Age:</label>"
				+ "<input type='text' name='reviewerage'>" + "</div>" + "<div class='col-md-4'>"
				+ "<label class='dark'>Gender:</label> " + "<input type='text' name='reviewergender'>" + "</div>"
				+ "<div class='col-md-4'>" + "<label class='dark'>Occupation:</label>"
				+ "<input type='text' name='revieweroccupation'>" + "</div>" + "</div>" + "</div>"
				+ "<div class='form-group'>" + "<label class='dark'>Comments:</label>"
				...
				for (int j = i; j < (i + 3); j++) {
					if (j < count) {
						_Content += "" + "<div class='col-xs-4'>" + "<table>" + "<tr>" + "<td>"
								...
								+ "</td>" + "</tr>" + "<tr>" + "<td>"
								+ "<label class='control-label green price'>Price: </label>"
	...
	private String getHTMLContent(String productId) {
		...
		for (Map.Entry<String, Product> entry : product.entrySet()) {
			...
			_Product += "<div class='row row-space'>" + "<div class='col-xs-3 img-block'>" + ""
					...
					+ "" + "</div>" + "<div class='col-xs-6 divide-block'>" + "<h4 class=''>" + "<a href='#'>"
					+ p.getModel() + "</a>" + "</h4>" + "<label class='control-label green'>Manufacturer: </label>"
					+ "<label class='info-label '>" + p.getManufacturer() + "</label>" + "<x class='space'></x> |"
					+ "<label class='control-label green space'>Condition: </label>" + "<label class='info-label '>"
					+ p.getCondition() + "</label>" + "<x class='space'></x> |"
					+ "<label class='control-label red space'>Discount: </label> " + "<label class='info-label'>"
					...
					+ "<form action='/CSJProj/AddToCart' name='requestedForm' value='AddToCart' method='post'>"
					+ "<input type='hidden' name='requestedForm' value='AddToCart' >"
					+ "<button class='btn btn-primary' name='cartProduct' value='" + p.getProductId()
