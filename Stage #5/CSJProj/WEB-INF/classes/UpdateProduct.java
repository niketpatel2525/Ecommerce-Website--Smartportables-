...
public class UpdateProduct extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (findProduct != null) {
			...
			if (resultcode == 1) {
				...
				out.println("<center><h6>Error occured in the transaction. Please try again later.</h6></center>");
			}
	...
	private void updateProduct(HttpServletRequest request, HttpServletResponse response, String src)
			throws IOException {
	...
	private String getProductView(Product p) {
		return "<div class='row'>" + "<div class='col-md-3'></div>"
				...
				+ "<input name='' value='"
				+ p.getProductId()
				+ "' disabled type='text' id='productId' class='form-control' placeholder='productId' required='' autofocus=''>"
				+ ""
				+ ""
				+ "<label for='model' class='sr-only'>Model Name</label>"
				+ "<input name='model' value='"
				+ p.getModel()
				+ "' type='text' id='model' class='form-control' placeholder='Model Name' required=''>"
				+ ""
				+ ""
				+ "<label for='price' class='sr-only'>Price</label>"
				+ "<input name='price' value='"
				+ p.getPrice()
				+ "' type='text' id='price' class='form-control' placeholder='Price' required=''>"
				+ ""
				+ ""
				+ "<label for='img' class='sr-only'>Image</label>"
				+ "Image:&nbsp <input name='img' value='"
				+ p.getImage()
				+ "' type='text' id='mno' class='form-control' required='' >"
				+ ""
				+ ""
				+ "<label for='manufacturer' class='sr-only'>Manufacturer</label>"
				+ "<input name='manufacturer' value='"
				+ p.getManufacturer()
				+ "' type='text' id='manufacturer' class='form-control' placeholder='Manufactured By' required=''>"
				+ ""
				+ ""
				+ "<label for='condition' class='sr-only'>Condition</label>"
				+ "<input name='condition' value='"
				+ p.getCondition()
				+ "' type='text' id='condition' class='form-control' placeholder='Condition' required=''>"
				+ ""
				+ ""
				+ "<label for='discount' class='sr-only'>Discount</label>"
				+ "<input name='discount' value='"
				+ p.getDiscount()
				+ "' type='text' id='discount' class='form-control'  placeholder='Discount' required=''>"
				+ ""
				+ ""
				+ "<label for='quantity' class='sr-only'>Quantity</label>"
				+ "<input name='quantity' value='"
				+ p.getQuantity()
				+ "' type='text' id='quantity' class='form-control' placeholder='Quantity' required=''>"
				...
				+ (p.getType().equals("watch") ? "<option value='watch' selected='selected'>Watch</option>"
						: "<option value='watch'>Watch</option>")
				+ (p.getType().equals("Speakers") ? "<option value='Speakers' selected='selected'>Speakers</option>"
						: "<option value='Speakers'>Speakers</option>")
				+ (p.getType().equals("Headphones")
						? "<option value='Headphones' selected='selected'>Headphones</option>"
						: "<option value='Headphones'>Headphones</option>")
				+ (p.getType().equals("Phones") ? "<option value='Phones' selected='selected'>Phones</option>"
						: "<option value='Phones'>Phones</option>")
				+ (p.getType().equals("Laptops") ? "<option value='Laptops' selected='selected'>Laptops</option>"
						: "<option value='Laptops'>Laptops</option>")
				+ (p.getType().equals("External Storage")
						? "<option value='External Storage' selected='selected'>External Storage</option>"
				...
				+ "<a class='btn btn-lg btn-danger btn-block' role='button' href='/CSJProj/UpdateProduct?DeleteProduct="
				+ p.getProductId()
				+ "' >Delete Product</a>"
				+ "</form>"
				+ "</div>"
	...
	private String getHTMLContent() {
		return "<div id='body'>" + "<section id='content'>"
				+ "<div class='row'>"
				+ "<div class='col-md-3'></div>"
				+ "<div class='col-md-6'>"
				+ "<div class='container'>"
