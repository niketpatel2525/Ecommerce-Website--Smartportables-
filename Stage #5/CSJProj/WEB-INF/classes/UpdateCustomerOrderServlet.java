...
public class UpdateCustomerOrderServlet extends HttpServlet {
		...
		if (orderDisplay != null) {
			...
			if (updateQuantity != null) {
				try {
					...
					// int resultCode = SerializeDataStoreManager.updateOrder(updateOrderId,
					// productId, quantity);
					MySqlDataStoreUtilities msdsq = new MySqlDataStoreUtilities();
	...
	private String getOrderView(Order ord) {
		...
		String _Header = "<div class='row'>" + "<div class='box'>"
				+ "<div class='box-header'>"
				+ "<h3 class='box-title'>Order Details <sub> ** Update product quantity one at a time</sub> </h3>"
				+ "<a class='btn btn-danger pull-right' role='button'  href='/CSJProj/CancelOrderServlet?cancelOrder="
				+ ord.getOrderId()
				+ "'    >Cancel Order</a>"
				+ "</div>"
				+ "<div class='row'>"
		...
		for (Map.Entry<Product, Integer> entry : cart.entrySet()) {
			...
			_Content += "" + "<form action='/CSJProj/UpdateCustomerOrderServlet?update=true' method='post'>"
					+ "<tr>"
					+ "<input type='hidden' name='updateOrderId' value ='"
					+ ord.getOrderId()
					+ "'>"
					...
					+ "</td>"
					+ "<td>"
					+ "<input type='text' size='3' name='quantity' value='"
					+ Quantity
					+ "'>"
					+ "</td>"
					+ "<td>"
	...
	private String getHTMLContent() {
		return "<div id='body'>" + "<section id='content'>"
				...
				+ "<div class='col-md-6'>"
				+ "<div class='container'>"
				+ "<form class='form-signin' method='post' action='/CSJProj/UpdateCustomerOrderServlet?order=display'>"
