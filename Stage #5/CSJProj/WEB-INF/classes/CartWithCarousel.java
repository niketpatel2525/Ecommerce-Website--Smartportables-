...
public class CartWithCarousel extends HttpServlet {
	...
	private String printCart() {
		String _Header = "<div id='body'>" + "<section id='content'>"
				+ (userLoggedIn ? "<form action='/CSJProj/PlaceOrderServlet' method='post'>"
						: "<form action='/CSJProj/LoginServlet' method='post'>")
				+ "<div class='box'>"
		...
		for (Map.Entry<Product, Integer> entry : cart.entrySet()) {
			...
			_Content += "<tr>" + "<td>"
					...
					+ Quantity
					+ "</td>"
					+ "<td><input type='checkbox' name='warranty' value='"
					+ p.getProductId()
					+ "'></td>"
					+ "<td><a href='/CSJProj/RemoveFromCart?requestedForm=RemoveFromCart&&cartProduct="
					+ p.getProductId()
					+ "' class='btn btn-danger' role='button'>Remove</a></td>";
			count++;

		}
		String _Footer = "</table>" + "</div>"
				+ "</div>"
				+ "</form>";
