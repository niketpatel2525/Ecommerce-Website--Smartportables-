				...
				for (int j = i; j < (i + 3); j++) {
					...
					if (j < count) {
						_Content += ""
								...
								+ "<button class='btn btn-primary' name='cartProduct' value='"+accessory.get(j).getProductId()+"'   type='submit'>Add to Cart</button>"
								+ "</form>"
								+ "</td>"
								+ "</tr>"
								+ "</table>"
  ...
  private String content_footer(){
	...
	private String printCart() {
		String _Header = "<div id='body'>"
				+ "<section id='content'>"
				+ (userLoggedIn ? "<form action='/CSJProj/PlaceOrderServlet' method='post'>":"<form action='/CSJProj/LoginServlet' method='post'>")
				+ "<div class='box'>"
		...
		for (Map.Entry<Product, Integer> entry : cart.entrySet()) {
			...
			_Content +="<tr>"
					...
					+ "<td><a href='/CSJProj/RemoveFromCart?requestedForm=RemoveFromCart&&cartProduct="+p.getProductId()+"' class='btn btn-danger' role='button'>Remove</a></td>";
			count++;
