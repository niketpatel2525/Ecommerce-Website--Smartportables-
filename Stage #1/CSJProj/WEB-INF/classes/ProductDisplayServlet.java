...
public class ProductDisplayServlet extends HttpServlet {
	...
	private String printProductCatalog(String type, String manufacturer) {
		String _Heading = ""
				+ "<div id='body'>"
				+ "<section id='content'>" + "<center><h2 class='form-signin-heading'>"
		...
		for (Map.Entry<String, ArrayList<Product>> entry : product.entrySet()) {
			...
			for (Product p : prod) {
				_Product += "<div class='row row-space'>" + "<div class='col-xs-3 img-block'>" + ""
						...
						+ "<button class='btn btn-primary' name='cartProduct' value='"+p.getProductId()+"'   type='submit'>Add to Cart</button>"
						+ "</td>"
						+ "</tr>"
						+ "<tr>"
						+ "<td>"
						+ "<a class='btn btn-success' role='button' href='/CSJProj/ViewProductServlet?productId="+p.getProductId()+"'>Review</a>"
						+ "</td>"
						+ "</tr>"
						+ "</table>"
						+ "</form>"
		...
		for (Map.Entry<String, ArrayList<Product>> entry : product.entrySet()) {
			...
			for (Product p : prod) {
				_Product += "<div class='row row-space'>"
								...
								+ ""
								+ "</div>"
								+ "<div class='col-xs-6 divide-block'>" + "<h4 class=''>" + "<a href='#'>" + p.getModel()
								+ "</a>"
						 		+ "</h4>" + "<label class='control-label green'>Manufacturer: </label>"
								+" <label class='info-label '>" + p.getManufacturer() + "</label>"
								+ "<x class='space'></x> |"
								+ "<label class='control-label green space'>Condition: </label>"
								+" <label class='info-label '>" + p.getCondition() + "</label>"
								...
								+ "<label class='control-label red space'>Discount: </label>"
								+" <label class='info-label'>" + p.getDiscount() + "% " + "</label> <br>"
								+ "<label " + (p.getQuantity() > 0 ? "class='availibility green'>Currently Available</label>" : "class='availibility red'>Currently Unavailable")
								+ "</div>"

								+ "<div class='col-xs-1 space-price-block '>"
								+ "<h3 class='price space'>$"	+ p.getPrice() + "</h3>"
								+ "<form action='/CSJProj/AddToCart' name='requestedForm' value='AddToCart' method='get'>"
								+ "<table>"
								+ "<tr>"
								+ "<td>"
